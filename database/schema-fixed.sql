-- MFA Relay Multi-Tenant Database Schema (FIXED VERSION)
...
-- Insert MFA Relay project with generated UUID
...
-- Allow users to be inserted into projects (for signup trigger)
DROP POLICY IF EXISTS "Users can be added to projects" ON project_users;
CREATE POLICY "Users can be added to projects" ON project_users
    FOR INSERT WITH CHECK (true);

-- MFA Email Accounts: Users can only access their own accounts in their projects
DROP POLICY IF EXISTS "Users can manage their own email accounts" ON mfa_email_accounts;
...
DROP POLICY IF EXISTS "Users can manage their own SMS config" ON mfa_sms_config;
...
DROP POLICY IF EXISTS "Users can view their own MFA codes log" ON mfa_codes_log;
...
-- Allow inserting MFA codes (for the API)
DROP POLICY IF EXISTS "API can insert MFA codes" ON mfa_codes_log;
CREATE POLICY "API can insert MFA codes" ON mfa_codes_log
...
-- Display created project for verification
SELECT 'MFA Relay project created with ID:' as message, id, name, slug
